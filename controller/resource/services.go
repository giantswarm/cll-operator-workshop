...
func (s *Services) EnsureCreated(ctx context.Context, obj interface{}) error {
	...
	s.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("currentReplicas = %d desiredReplicas = %d", currentReplicas, desiredReplicas))
...
func (s *Services) EnsureDeleted(ctx context.Context, obj interface{}) error {
	...
	s.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("currentReplicas = %d", currentReplicas))

	if currentReplicas == 0 {
		s.logger.LogCtx(ctx, "level", "debug", "message", "no created services")
		return nil
	}
...
func (s *Services) ensureReplicaCreated(ctx context.Context, m *workshopv1alpha1.MemcachedConfig, replica int) error {
	...
	if current == nil {
		...
		s.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("service %s/%s created", desired.Namespace, desired.Name))
	} else {
		...
		s.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("service %s/%s updated", desired.Namespace, desired.Name))
	}

	return nil
}

func (s *Services) ensureReplicaDeleted(ctx context.Context, m *workshopv1alpha1.MemcachedConfig, replica int) error {
	...
	if apierrors.IsNotFound(err) {
		s.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("service %s/%s already deleted", namespace, name))
	} else if err != nil {
		return microerror.Mask(err)
	} else {
		s.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("service %s/%s deleted", namespace, name))
	}

	return nil
}

func newDesiredService(memcachedConfig *workshopv1alpha1.MemcachedConfig, replica int) (*corev1.Service, error) {
	name := key.ReplicaName(replica)
